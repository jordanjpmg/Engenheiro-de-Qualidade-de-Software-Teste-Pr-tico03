...
public class BasePage {
	...
	public boolean waitForTwoElements(WebElement element1, WebElement element2 , int timeOutInSeconds) {
		...
		try {
			...
			Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(timeOutInSeconds, TimeUnit.SECONDS)
					.pollingEvery(200, TimeUnit.MILLISECONDS).ignoring(NoSuchElementException.class)
					.ignoring(StaleElementReferenceException.class).ignoring(TimeoutException.class);
			webElement = wait.until(new Function<WebDriver, Boolean>() {
				public Boolean apply(WebDriver driver) {
					return exist(element1) | exist(element2);
				}
			});
		} catch (NoSuchElementException e) {
			return false;
	...
	protected boolean waitNotPresent(WebElement element, int timeOutInSeconds) {
		...
		boolean isElementInvisible = false;
		try {
			isElementInvisible = wait.until(ExpectedConditions.invisibilityOf(element));
		} catch (Exception e) {
			return false;
		}
		return isElementInvisible;
...
//	}
	...
	public static Boolean exist(By by, int timeout) {
	...
	public void scrollToBottomPage(int i){
		JavascriptExecutor js = (JavascriptExecutor) driver;
	...
	public void zoomOut(String percent){
		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("document.body.style['-webkit-transform'] = \"scale("+ percent +")\";");
	}

	public String getPropretie(String data){
